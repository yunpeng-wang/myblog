    ...
    const themes = [
        {
            ...
            birds: [
                {
                    light: { src: '/images/main_bg/crane_light.webp', x: 0.88, y: 0.73, width: 80 },
                    dark: { src: '/images/main_bg/crane_dark.webp', x: -0.18, y: 0.72, width: 80 }
                },
                {
                    light: { src: '/images/main_bg/twobirds_light.webp', x: -0.3, y: 0.3, width: 80 },
                    dark: { src: '/images/main_bg/twobirds_dark.webp', x: 1.0, y: 0.3, width: 80 }
                }
            ]
        },
        {
    ...
    function updateBackground() {
        if (!(isMobile())) {
            ...
            sel_theme.birds.forEach(b => {
                ...
                img.style.opacity = 0;
                img.style.animation = "none";  // 清除上次动画（保险）
                img.offsetHeight;              // 强制重绘
                img.style.animationDelay = `${Math.random() * 0.8}s`;
                img.style.animation = "imgFade 1.5s ease-in-out forwards";
